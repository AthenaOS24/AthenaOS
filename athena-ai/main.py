...
app = FastAPI(
    ...
    version="7.1.0",
    description="Advanced AI therapist with Cognitive Behavioral Therapy integration and Emotion Chart generation."  
)

# Phục vụ file tĩnh (ảnh) từ thư mục 'static'
...
class ChatResponse(BaseModel):
    ...
    chart_image_url: Optional[str] = Field(None, description="URL to the emotion analysis chart")
...
async def read_root():
    ...
    return {
        "status": "Athena AI API (v7.1.0 with Chart Generation) is running",
        "version": "7.1.0",
        "features": ["CBT Pattern Detection", "Crisis Detection", "Emotion Chart Generation", "Groq/Llama-3 Powered"]  
    }
...
"""
    ...
    try:
        ...
        chart_url = None
        if analysis_result_dict.get('emotions'):
            logger.info("Generating emotion chart...")
            chart_url = generate_emotion_chart(analysis_result_dict['emotions'], session_id)
