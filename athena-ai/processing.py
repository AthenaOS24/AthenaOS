...
def generate_emotion_chart(emotions_data: list, session_id: str):
    """
    Tạo biểu đồ cảm xúc từ dữ liệu và lưu lại dưới dạng file ảnh.
    Trả về đường dẫn tương đối tới file ảnh.
    """
    if not emotions_data:
        return None

    # Tạo thư mục 'static' nếu chưa có để lưu ảnh
    static_dir = "static"
    if not os.path.exists(static_dir):
        os.makedirs(static_dir)

    # Sắp xếp cảm xúc theo điểm số giảm dần và lấy top 5
    top_emotions = sorted(emotions_data, key=lambda x: x['score'], reverse=True)[:5]

    labels = [e['label'].capitalize() for e in top_emotions]
    scores = [e['score'] for e in top_emotions]

    # Bắt đầu vẽ
    fig, ax = plt.subplots(figsize=(8, 5)) # Kích thước biểu đồ
    bars = ax.barh(labels, scores, color='#339AF0') # Vẽ biểu đồ cột ngang

    # Tùy chỉnh cho đẹp hơn
    ax.set_xlabel('Mức độ', fontsize=12)
    ax.set_title('Phân Tích Cảm Xúc', fontsize=14, fontweight='bold')
    ax.invert_yaxis()  # Đưa cảm xúc cao nhất lên trên
    ...
    # Lưu file ảnh với tên duy nhất
    chart_filename = f"{session_id}_{random.randint(1000,9999)}_emotion_chart.png"
    chart_path = os.path.join(static_dir, chart_filename)
    plt.savefig(chart_path)
    plt.close(fig) # Đóng biểu đồ để giải phóng bộ nhớ

    logger.info(f"Generated emotion chart: {chart_path}")
    
    # Trả về URL tương đối để truy cập
    return f"/{static_dir}/{chart_filename}"