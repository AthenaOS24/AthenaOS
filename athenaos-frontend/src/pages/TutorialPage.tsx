...
const DEFAULT_CHECKLIST: ChecklistItem[] = [
  ...
  { key: "backend", label: "Backend running (Express + PostgreSQL reachable)", done: false },
  { key: "frontend", label: "Frontend builds & runs locally", done: false },
  { key: "auth", label: "Register & Login tested (happy path)", done: false },
      ...
      <Tabs defaultValue="overview" className="page-tabs" keepMounted={false}>
        ...
        <Tabs.Panel value="setup" pt="md">
          <Card className="page-card" withBorder padding="lg" radius="md">
            ...
            <Text mt="xs">
              In <Code>athenaos-backend</Code>:
            </Text>
            <Paper className="code-block" withBorder p="sm" radius="md">
              <Code block>{`# from project root
cd athenaos-backend
cp .env.example .env  # ensure DB + FRONTEND_URL are correct
npm install           # or pnpm i
npm run dev           # http://localhost:8888 by default`}</Code>
            </Paper>
            ...
            <Text mt="xs">
              In <Code>athenaos-frontend</Code>:
            </Text>
            <Paper className="code-block" withBorder p="sm" radius="md">
              <Code block>{`cd ../athenaos-frontend
cp .env.example .env  # set VITE_API_BASE=http://localhost:8888
npm install           # or pnpm i
npm run dev           # http://localhost:5173`}</Code>
            </Paper>

            <Alert mt="md" variant="light" color="blue" icon={<IconInfoCircle />}>
              If login fails, test the API directly (e.g., with <Code>curl</Code> or Postman) to isolate FE vs BE.
            </Alert>
            
            <Alert mt="md" variant="light" color="orange" title="Important Note on Registration" icon={<IconAlertTriangle />}>
